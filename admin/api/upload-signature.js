...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (missing.length > 0) {
      console.error('[Cloudinary Signature API Error] Missing environment variables in Admin Vercel environment:', missing.join(', '))
      return res.status(500).json({
        error: 'Cloudinary server configuration is incomplete.',
        details: `Missing environment variable(s): ${missing.join(', ')} in Vercel Admin environment.`,
      })
    }

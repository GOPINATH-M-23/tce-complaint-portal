...
const NAV = [
  ...
  { to: '/admin/complaints', label: 'All Complaints', icon: '≡', badge: true },
...
export default function AdminSidebar() {
  ...
  const handleLogout = async () => {
    ...
    toast.success('Logged out'); navigate('/')
  }
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {NAV.map(({ to, label, icon, badge }) => (
          <NavLink key={to} to={to} end={to === '/admin'}
            ...
            <span className="w-5 text-sm leading-none opacity-80">{icon}</span>
      ...
      <div className="mt-auto pt-5 border-t border-white/10">
        <div className="flex items-center gap-2 mb-3 min-w-0">
          ...
          <div className="overflow-hidden min-w-0">
            ...
            <div className="text-[11px] text-white/50 truncate">{user?.role}</div>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={toggle} aria-label="Toggle dark mode"
            className="relative flex items-center justify-between w-12 h-6 rounded-full px-1 border-0 cursor-pointer transition-colors duration-300"
            style={{ background: dark ? '#3d8c6a' : 'rgba(255,255,255,0.2)' }}>
            <span className="text-[10px] text-white/70 select-none">☀</span>
            <span className="text-[10px] text-white/70 select-none">☽</span>

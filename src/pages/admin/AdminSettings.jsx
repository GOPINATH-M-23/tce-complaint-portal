...
export default function AdminSettings() {
  ...
  const handleExport = () => {
    // ID excluded from CSV export (backend only)
  ...
  return (
    <div className="max-w-xl space-y-5 md:space-y-6 mt-4">
      <h1 className="font-display text-xl md:text-2xl font-bold text-tce-dark dark:text-white">Settings</h1>
      ...
      <Section title="Admin Profile">
        <div className="flex items-center gap-4 mb-5 pb-5 border-b border-tce-dark/[0.06] dark:border-gray-700/50">
          ...
          <div className="min-w-0">
            <div className="font-semibold text-tce-dark dark:text-white truncate">{user?.name}</div>
            <div className="text-sm text-gray-500 dark:text-gray-400 truncate">{user?.email}</div>
            <div className="text-xs text-tce-green mt-0.5">{user?.role} · {user?.dept}</div>
          </div>
        </div>
        {[['Name', user?.name], ['Email', user?.email], ['Role', user?.role], ['Department', user?.dept]]
          .map(([k, v]) => (
        ...
        <Row
          ...
          action={
            <button onClick={toggle} aria-label="Toggle dark mode"
              className="relative flex items-center justify-between w-14 h-7 rounded-full px-1.5 border-0 cursor-pointer transition-colors duration-300 shrink-0"
              ...
              <span className="absolute top-[3px] left-[3px] w-[22px] h-[22px] rounded-full bg-white shadow-sm transition-transform duration-300"
      ...
      <Section title="Data Export">
        <Row
          label="Export All Complaints"
          sub={`Download as CSV (${complaints.length} complaints)`}
          action={
            <button className="btn-primary text-sm shrink-0" onClick={handleExport}>
              Export CSV
            </button>
          }
        />
